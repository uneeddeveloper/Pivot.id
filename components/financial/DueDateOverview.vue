    ...
    <ul class="divide-y divide-ink-100">
      ...
      >
        ...
        <div class="flex shrink-0 flex-col items-end gap-1.5">
          ...
          <span v-if="item.debt.minPayment > 0" class="text-xs tabular-nums text-ink-500">
    ...
    <template #footer>
      <div class="space-y-2">
        <div v-if="financial.dueThisWeek > 0" class="flex items-baseline justify-between gap-3">
          <span class="text-sm text-ink-600">Perlu disiapkan dalam 7 hari</span>
          <span class="text-sm font-semibold tabular-nums text-brand-700">
            {{ formatIDR(financial.dueThisWeek) }}
          </span>
        </div>

        <p v-if="overdueCount > 0" class="attention-note text-xs leading-relaxed">
