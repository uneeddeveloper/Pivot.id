...
<template>
  <section
    ...
    class="surface-brand relative overflow-hidden rounded-2xl border border-brand-200 dark:border-brand-700/60 p-6 text-brand-900 dark:text-cream-100"
  >
    <div class="flex flex-wrap items-start justify-between gap-5">
      <div class="min-w-0">
        <p class="text-xs font-medium tracking-wide text-brand-700 dark:text-cream-100/75 uppercase">
          {{ plan.covered ? 'Rencana ini menutup kebutuhanmu' : 'Yang bisa kamu kumpulkan minggu ini' }}
        </p>
        <p class="mt-2 text-3xl font-bold tracking-tight tabular-nums text-brand-900 dark:text-cream-50 sm:text-4xl">
          {{ formatIDR(plan.estimatedMin) }}
        </p>
        <p class="mt-1 text-sm text-brand-800 dark:text-cream-100/80">
    ...
    <ol class="mt-6 space-y-2.5">
      <li
        ...
        :key="step.gig.id"
        class="flex items-start gap-3 rounded-xl border border-brand-300/40 dark:border-cream-100/20 bg-brand-50/50 dark:bg-cream-100/10 px-4 py-3"
      >
        <span
          class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-200 dark:bg-cream-100/20 text-[11px] font-semibold text-brand-800 dark:text-cream-50"
        >
          {{ index + 1 }}
        </span>
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-brand-900 dark:text-cream-50">
            {{ step.units }}× {{ step.gig.title }}
          </p>
          <p class="mt-0.5 text-xs text-brand-700 dark:text-cream-100/70">
            {{ step.gig.unit }} · ± {{ hours(step.hours) }}
          </p>
        </div>
        <p class="shrink-0 text-sm font-semibold tabular-nums text-brand-900 dark:text-cream-50">
    ...
    <div
      ...
      class="mt-5 rounded-xl border border-brand-300/40 dark:border-cream-100/25 bg-brand-50/50 dark:bg-cream-100/10 px-4 py-3"
    >
      <p class="text-sm leading-relaxed text-brand-900 dark:text-cream-50">
        Masih kurang
      ...
      -->
      <p v-if="plan.beyondReach" class="mt-1 text-sm leading-relaxed text-brand-800 dark:text-cream-100/85">
        ...
        Yang lebih masuk akal: <strong class="font-semibold text-brand-900 dark:text-cream-50">pecah targetnya ke
      ...
      <p
        ...
        class="mt-1 text-sm leading-relaxed text-brand-800 dark:text-cream-100/85"
      >
        Dengan tambahan sekitar
        <strong class="font-semibold text-brand-900 dark:text-cream-50">{{ hours(plan.extraHoursNeeded) }}</strong>
    ...
    <p class="mt-5 text-xs leading-relaxed text-brand-700 dark:text-cream-100/70">
  ...
  <BaseCard v-else-if="plan.need > 0" tone="soft">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
      ...
      <div class="min-w-0">
        <p class="font-semibold text-ink-900 dark:text-cream-50">
