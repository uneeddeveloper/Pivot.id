...
<template>
  <article class="bg-white/40 dark:surface-dark-card rounded-[1.25rem] border border-ink-200/50 dark:border-white/[0.08] p-5 sm:p-6 transition-all duration-400 hover:border-ink-300 dark:hover:border-white/[0.2] lift-dark">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div class="min-w-0">
        <div class="flex flex-wrap items-center gap-2">
          <h3 class="font-semibold text-ink-900 dark:text-cream-50">{{ match.role.title }}</h3>
          <span
            class="rounded-full bg-ink-200/50 dark:bg-white/[0.05] px-2 py-0.5 text-[10px] font-medium tracking-wide text-ink-600 dark:text-ink-400 uppercase border border-ink-200/50 dark:border-white/[0.05]"
          >
            {{ match.role.field }}
          </span>
        </div>
        <p class="mt-1.5 text-sm leading-relaxed text-ink-600 dark:text-ink-400">{{ match.role.description }}</p>
      </div>
    ...
    <p v-if="targetIncome > 0 && !match.meetsTarget" class="mt-1 text-xs text-ink-600 dark:text-ink-500">
      Masih kurang
      <strong class="font-semibold text-ink-900 dark:text-cream-100">{{ formatIDR(Math.abs(match.gapToTarget)) }}</strong>
      dari Target Income-mu — bisa jadi pijakan sementara sambil mengejar peran berikutnya.
    </p>
    <p v-else-if="targetIncome > 0" class="mt-1 text-xs text-sage-600 dark:text-sage-400">
    ...
    <div class="mt-4 space-y-2.5">
      ...
      <div v-if="match.missing.length" class="flex flex-wrap items-center gap-1.5">
        <span class="mr-0.5 text-xs text-ink-600 dark:text-ink-500">Perlu dikejar:</span>
        <span
      ...
      <p v-if="!match.missing.length" class="text-xs font-medium text-sage-600 dark:text-sage-400">
        Semua keterampilan intinya sudah kamu punya — peran ini bisa langsung kamu lamar.
      </p>
    </div>

    <div class="mt-4 flex flex-wrap items-center gap-1.5 border-t border-ink-200/50 dark:border-white/[0.08] pt-3">
      <span
      ...
      <NuxtLink
        ...
        class="focus-ring dark:focus-ring-dark ml-auto shrink-0 rounded-lg px-2 py-1 text-xs font-medium text-brand-600 dark:text-brand-400 transition hover:bg-brand-50 dark:hover:bg-brand-500/10"
      >
        Lihat lowongannya →
      </NuxtLink>
    </div>
